...
APP_VERSION = "fx_rate8_stable_local_csv_2026-06-16"

# リポジトリ内の quote.csv を直接読む
QUOTE_CSV_PATH = Path(__file__).resolve().parent / "quote.csv"
...
def read_quote_csv_text() -> str:
    """
    quote.csv をローカルファイルとして読み込む。
    文字コードは代表的な候補を順に試す。
    """
    if not QUOTE_CSV_PATH.exists():
        raise FileNotFoundError(
            f"quote.csv が見つかりません。配置場所: {QUOTE_CSV_PATH}"
        )

    raw = QUOTE_CSV_PATH.read_bytes()

    # みずほCSV想定で Shift_JIS 系を優先
    ...
    raise UnicodeDecodeError(
        ...
        0,
        1,
        f"quote.csv の文字コードを判定できませんでした。最後のエラー: {last_error}",
    )


def parse_quote_csv(text: str) -> pd.DataFrame:
    """
    みずほ quote.csv を DataFrame 化する。
    ヘッダー行の位置が多少変わっても DATE 列を推定して吸収する。
    """
    # まずはヘッダーなしで仮読み
    ...
    if header_idx is None:
        # ヘッダー行が見つからない場合は通常読込を試す
        df = pd.read_csv(io.StringIO(text), encoding="shift_jis")
        cols = [str(c).strip() for c in df.columns]
        if not cols:
            raise ValueError("quote.csv の列見出しを読み取れませんでした。")
    ...
    if df.empty:
        raise ValueError("quote.csv は読み込めましたが、有効なDATE行がありません。")
...
def load_quote_df() -> pd.DataFrame:
    """
    外部HTTPアクセスはせず、リポジトリ内の quote.csv を直接読む。
    """
    text = read_quote_csv_text()
...
def get_avg_ttm_simple(df: pd.DataFrame, ccy: str, start_d: date, end_d: date) -> float:
    ...
    sel = tmp.loc[
        (tmp["DATE_ONLY"] >= start_d) & (tmp["DATE_ONLY"] <= end_d),
        "TTM"
    ].dropna()
...
def build_print_html(start_d: date, end_d: date, ccy: str, avg: float, note: str) -> str:
    fixed_sentence = "レートの証拠として精算書にこの書面を添付してください。"
...
def main() -> None:
    st.set_page_config(page_title="出張期間の平均レート", layout="centered")
    init_session_state()
    ...
    with st.expander("（確認用）稼働情報", expanded=False):
        ...
        st.write("CSV_PATH:", str(QUOTE_CSV_PATH))
    ...
    st.caption("プルダウンに出ない通貨の場合には、メールで経理及川宛てにレートの問い合わせをしてください")
    foreign = st.selectbox("外貨（対JPY）", TARGET_CCYS, index=0)

    if st.button("平均レート計算"):
        try:
            if start_date > end_date:
                raise ValueError("開始日が終了日より後になっています。日付を確認してください。")
